# -*- coding: utf-8 -*-
"""
Created on Wed Jun 22 17:20:01 2022
...
ml= pd.read_csv (r'C:\Users\cmancera\OneDrive - Apex Systems\Glassdoor\Final Output\glassdoor_english_Machine,Learning,Engineer_22-06-2022_1449.csv')
...
data["Date_scrapped"]= datetime(2022, 6, 22)
...
for i in range(len(data)):
    ...
    elif data["Date_published"][i] == '30 días o más':
        data["Date_published_final"][i]=(data["Date_scrapped"][i]- timedelta(days=45)).strftime(r"%d/%m/%Y")
    elif data["Date_published"][i][-5:] == "horas":
        dias= round(int(data["Date_published"][i][:-6])/24)
        data["Date_published_final"][i]=(data["Date_scrapped"][i]- timedelta(days=dias)).strftime(r"%d/%m/%Y")
    else:
...
for i in range(len(data)):
    ...
    if null_sal:
        pass
    elif('M' in data["Salary"][i][:6]):
        data["Salary_min"][i]= int(range_sal[0])/12*1000000
    else: 
        data["Salary_min"][i]= int(range_sal[0])/12*1000
        
        
    #if salary high range
    if null_sal:
        pass
    elif ('M' in data["Salary"][i][7:]):
        try:
            data["Salary_max"][i]= int(range_sal[1])/12*1000000
        except:
            data["Salary_max"][i]= int(range_sal[0])/12*1000000
    else: 
        try:
            data["Salary_max"][i]= int(range_sal[1])/12*1000
        except:
            data["Salary_max"][i]= int(range_sal[0])/12*1000
...
duplicadas_sum= sum(duplicates)
print(duplicadas_sum)
